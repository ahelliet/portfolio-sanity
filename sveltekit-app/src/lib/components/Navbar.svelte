<script lang="ts">
	...
	import type { NavLink, LayoutData } from "$lib/sanity/queries";
	...
	let leftLinks = $derived(
		(layout.navLinks ?? []).filter((link) => !link.rightSide)
	);
	...
	<div class="flex items-center justify-between py-5 px-20">
		...
		<div class="flex items-center gap-12">
			<a href="/" class="flex items-center">
				{#if layout.logo}
					<img
						src={urlFor(layout.logo).height(28).url()}
						alt={layout.title ?? "Logo"}
						class="h-7 rounded"
					/>
				{:else}
					<span class="font-serif italic text-[28px] text-foreground">
						{layout.title ?? ""}
					</span>
				{/if}
			</a>
			...
			{#if leftLinks.length > 0}
				<div class="hidden items-center gap-8 md:flex">
					{#each leftLinks as link (link.label)}
		...
		<div class="flex items-center gap-4">
			...
			{#if layout.socialLinks && layout.socialLinks.length > 0}
				<div class="flex items-center gap-4">
					{#each layout.socialLinks as social (social.platform)}
						<a
							href={social.url}
							target="_blank"
						...
						>
							...
							{:else}
								<span class="text-xs uppercase">{social.platform}</span>
							{/if}
						</a>
					{/each}
				</div>
